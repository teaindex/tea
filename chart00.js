...
function redraw_chart() {
	...
	if (label2=="DayIndex") {
		...
		for (var i=0;i<jsondata[genre].length;i++) {
			...
			if (genre=="TwoIndex") {
				if (parent!=dict1[jsondata[genre][i]["belong2id"]])
					continue;
				//console.log(jsondata[genre][i]);
			}
			else if (genre=="ThreeIndex") {
				if (parent!=dict2[jsondata[genre][i]["belong2id"]])
					continue;
				//console.log(jsondata[genre][i]);
			}
			...
			if (flag==-1) {
				...
				if (genre=="OneIndex") {
					dict1[jsondata[genre][i]["id"]] = thisName;
				}
				else if (genre=="TwoIndex") {
					dict2[jsondata[genre][i]["id"]] = thisName;
				}
			}
			//console.log(flag);
...
function loadDayIndex() {
	...
	$.get("http://cccqqf.top:8081/test/salesandindex",function(data,status) {
		//console.log(data);
		jsondata["AllIndex"] = data["AllIndex"];
		jsondata["OneIndex"] = data["OneIndex"];
		jsondata["TwoIndex"] = data["TwoIndex"];
		jsondata["ThreeIndex"] = data["ThreeIndex"];
